...
describe('WorkerSystem', () => {
  ...
  describe('serialize / deserialize', () => {
    it('round-trips worker stats through serialize → deserialize', () => {
      system.syncPopulation(5);
      // Mutate some stats to ensure they survive the round-trip
      const allStats = [...system.getStatsMap().entries()];
      allStats[0]![1].morale = 77;
      allStats[0]![1].skill = 42;
      allStats[0]![1].vodkaDependency = 33;
      allStats[0]![1].ticksSinceVodka = 7;
      allStats[0]![1].assignmentDuration = 12;

      const data = system.serialize();
      expect(data.workers).toHaveLength(5);

      // Deserialize into a new system (citizens are still in ECS)
      const rng2 = new GameRng('test-worker-seed');
      const restored = WorkerSystem.deserialize(data, rng2);

      // All 5 citizens should have stats re-linked
      const restoredStats = [...restored.getStatsMap().entries()];
      expect(restoredStats).toHaveLength(5);

      // Find the mutated worker by name
      const originalName = allStats[0]![1].name;
      const found = restoredStats.find(([, s]) => s.name === originalName);
      expect(found).toBeDefined();
      expect(found![1].morale).toBe(77);
      expect(found![1].skill).toBe(42);
      expect(found![1].vodkaDependency).toBe(33);
      expect(found![1].ticksSinceVodka).toBe(7);
      expect(found![1].assignmentDuration).toBe(12);
    });

    it('serialized data contains correct class per worker', () => {
      system.syncPopulation(3);
      const data = system.serialize();
      ...
      for (const entry of data.workers) {
        expect(validClasses).toContain(entry.class);
      }
    });

    it('deserialize preserves worker names', () => {
      system.syncPopulation(4);
      const data = system.serialize();
      const originalNames = data.workers.map((w) => w.stats.name).sort();

      const rng2 = new GameRng('test-worker-seed');
      const restored = WorkerSystem.deserialize(data, rng2);
      const restoredNames = [...restored.getStatsMap().values()].map((s) => s.name).sort();

      expect(restoredNames).toEqual(originalNames);
    });

    it('serialized data is a plain object (no entity references)', () => {
      system.syncPopulation(2);
      const data = system.serialize();
      const json = JSON.stringify(data);
      const parsed = JSON.parse(json);
      expect(parsed.workers).toHaveLength(2);
      expect(parsed.workers[0].stats.name).toBeTruthy();
    });

    it('deserialized system can still tick normally', () => {
      system.syncPopulation(3);
      const data = system.serialize();

      const rng2 = new GameRng('test-worker-seed');
      const restored = WorkerSystem.deserialize(data, rng2);
      const result = restored.tick(100, 100);

      expect(result.vodkaConsumed).toBeGreaterThanOrEqual(0);
      expect(result.foodConsumed).toBeGreaterThanOrEqual(0);
    });
    ...
    it('handles empty worker list', () => {
      ...
      const rng2 = new GameRng('test-worker-seed');
      const restored = WorkerSystem.deserialize(data, rng2);
      expect([...restored.getStatsMap().entries()]).toHaveLength(0);
    });
  });
