...
describe('powerSystem', () => {
  ...
  describe('power distribution', () => {
    it('distributes power to buildings within total capacity', () => {
      ...
      createBuilding(1, 1, 'housing'); // 5 req
      createBuilding(2, 2, 'farm'); // 2 req
      powerSystem();
      const store = getResourceEntity()!;
  ...
  describe('power usage tracking', () => {
    ...
    it('tracks total power used correctly with multiple consumers', () => {
      createBuilding(0, 0, 'power'); // 100 output
      createBuilding(1, 1, 'housing'); // 5 req
      createBuilding(2, 2, 'housing'); // 5 req
      createBuilding(3, 3, 'farm'); // 2 req
      createBuilding(4, 4, 'distillery'); // 5 req
      powerSystem();
      const store = getResourceEntity()!;
  ...
  describe('edge: more buildings than power capacity', () => {
    it('leaves excess buildings unpowered (first-come first-served)', () => {
      ...
      const gulags = [];
      for (let i = 0; i < 11; i++) {
        gulags.push(createBuilding(i + 1, i + 1, 'gulag'));
      }
      powerSystem();
    ...
    it('does not count unpowered buildings in powerUsed', () => {
      ...
      createBuilding(0, 0, 'power');
      for (let i = 0; i < 3; i++) {
        createBuilding(i + 1, 0, 'housing');
      }
      for (let i = 0; i < 10; i++) {
        createBuilding(i + 4, 0, 'gulag');
      }
      powerSystem();
