/**
 * @module ai/agents/core/weather-types
 *
 * Weather type enum, profile interface, and WEATHER_PROFILES constant.
 * Shared by WeatherAgent, ChronologyAgent, DefenseAgent, SimulationEngine, and UI.
 *
 * Originally in game/WeatherSystem.ts — moved here as canonical location.
 */
...
export function rollWeather(season: Season, rng: GameRng): WeatherState {
  ...
  if (!weights) {
    return { current: WeatherType.OVERCAST, daysRemaining: 2 };
  }
...
/** Returns the weather profile for the given weather type. */
