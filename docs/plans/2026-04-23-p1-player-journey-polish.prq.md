...
**Shipped (21/24):** P1A-1..5, P1B-2..3, P1C-1..3, P1D-2..3, P1E-1..2, P1F-1..6.
Merged in PRs #55–#75 plus infra #74.

**In-flight (2):**
- P1B-1 (toast coalescing) — PR #76 open, core CI pending.
- P1D-1 (store screenshots + metadata) — PR #77 open, core CI pending.

**Deferred (1):** P1D-4 release dry-run — procedural, runs next time a
release-please PR merges to exercise the full chain end-to-end.
...
- [x] **P1A-1** Backfill `collectivization` era events
...
- [x] **P1A-2** Backfill `reconstruction` era events
...
- [x] **P1A-3** Backfill `thaw_and_freeze` era events
...
- [x] **P1A-4** Audit and resolve `victoryCondition` fields on 5 eras
...
- [x] **P1A-5** Document or fix `great_patriotic` empty `unlockedBuildings`
...
- [x] **P1B-2** Rescale HQ tab population gates
...
- [x] **P1B-3** Update `docs/design/ui-ux.md` to match shipped UI
...
- [x] **P1C-1** Delete 26 out-of-scope GLBs from `assets/models/soviet/`
...
- [x] **P1C-2** Remove dead shader declarations from `assets/polyhaven-requirements.json`
...
- [x] **P1C-3** Clean up stray PNG screenshots from repo root
...
- [x] **P1D-2** Document Android signed release path
...
- [x] **P1D-3** Document iOS signed release path
...
- [x] **P1F-1** HUD typography: adopt IBM Plex Mono throughout game UI
...
- [x] **P1F-2** Resolve directive/tutorial first-instruction conflict
...
- [x] **P1F-3** IntroModal copy: explain organic settlement growth
...
- [x] **P1F-4** Missing classified-archive era intros
...
- [x] **P1F-5** Explain Great Patriotic War construction freeze in-game
...
- [x] **P1F-6** Dev-mode SQLite silent failure: surface a clear developer message
...
- [x] **P1E-1** Fix runtime deprecation warnings
...
- [x] **P1E-2** Extend browser campaign smoke coverage
