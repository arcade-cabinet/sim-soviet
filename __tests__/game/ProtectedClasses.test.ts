import { PROTECTED_CLASSES, isProtected, getDemolitionPriority, type BuildingProtectionClass } from '../../src/config/protectedClasses';

describe('Protected Classes', () => {
  it('government is never demolished', () => { expect(isProtected('government')).toBe(true); });
  it('military is never demolished', () => { expect(isProtected('military')).toBe(true); });
  it('housing is fully expendable', () => { expect(isProtected('housing')).toBe(false); });
  it('farms are fully expendable', () => { expect(isProtected('farms')).toBe(false); });
