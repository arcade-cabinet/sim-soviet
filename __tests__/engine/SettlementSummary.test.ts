import { buildSettlementSummary } from '../../src/game/engine/SettlementSummary';
...
describe('SettlementSummary', () => {
  ...
  it('is the same shape regardless of game year', () => {
    const early = buildSettlementSummary({
      ...
      yearsSinceLastDisaster: Infinity,
    });
    const late = buildSettlementSummary({
      year: 9999,
      month: 12,
      ...
      trendDeltas: { food: -100, population: -500, morale: -5, power: -200 },
      yearsSinceLastWar: 0,
      yearsSinceLastFamine: 0,
      yearsSinceLastDisaster: 50,
    });
